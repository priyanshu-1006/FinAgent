// ===== DOM Elements =====
// Initialize as empty, will be populated on DOMContentLoaded
let pages = {};
let modals = {};
...
// Back buttons
document
  .getElementById("bills-back")
  .addEventListener("click", () => showPage("dashboard"));
document
  .getElementById("transfer-back")
  .addEventListener("click", () => showPage("dashboard"));
document
  .getElementById("gold-back")
  .addEventListener("click", () => showPage("dashboard"));
document
...
billAmount.addEventListener("input", () => {
  ...
  if (amount > 0) {
    document.getElementById("summary-biller").textContent =
      document.getElementById("biller-select").options[
        document.getElementById("biller-select").selectedIndex
      ].text;
...
transferAmount.addEventListener("input", () => {
  ...
  if (amount > 0) {
    ...
    document.getElementById("transfer-amount-display").textContent =
      formatCurrency(amount);
...
goldAmount.addEventListener("input", () => {
  ...
  document.querySelector("#grams-hint strong").textContent =
    grams.toFixed(3) + " grams";

  if (amount > 0) {
    document.getElementById("gold-quantity").textContent =
      grams.toFixed(3) + " grams";
...
goldGrams.addEventListener("input", () => {
  ...
  document.querySelector("#amount-hint strong").textContent =
    formatCurrency(amount);
...
goldForm.addEventListener("submit", async (e) => {
  ...
  document.getElementById("confirm-message").textContent =
    "Please review the details before proceeding:";
  document.getElementById("confirm-details").innerHTML = `
        <p><strong>Gold Rate:</strong> ${formatCurrency(
          appState.goldRate
        )} / gram</p>
  ...
  .addEventListener("click", async () => {
    ...
    switch (action.type) {
      ...
      case "buy-gold":
        ...
        document.getElementById("gold-holdings").textContent =
          appState.goldHoldings.toFixed(3) + " grams";
        addTransaction("Digital Gold Purchase", action.amount, true);
        document.getElementById("success-title").textContent =
          "Gold Purchased!";
        document.getElementById("success-message").textContent =
          "Gold has been added to your holdings.";
        document.getElementById("success-details").innerHTML = `
                <p><strong>Quantity:</strong> ${action.grams.toFixed(
                  3
                )} grams</p>
                <p><strong>Amount:</strong> ${formatCurrency(action.amount)}</p>
                <p><strong>Total Holdings:</strong> ${appState.goldHoldings.toFixed(
                  3
                )} grams</p>
            `;
        goldForm.reset();
...
function init() {
  ...
  // Ensure only login page is visible initially
